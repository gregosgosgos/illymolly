...
> 브라우저는 `copy` · `cut` · `paste` 이벤트를 통해서만 시스템 클립보드를 내줍니다.
> 그래서 `Ctrl+C/X/V` 는 단축키 처리에서 막지 않고 브라우저에 넘긴 뒤,
> 돌아온 이벤트에서 처리합니다 (`src/clipboard.js`). 메뉴에서 누른 [붙이기] 는
> 이벤트가 없으므로 비동기 Clipboard API 로 읽고, 막히면 앱 안 클립보드로 되돌아갑니다.
...
| 스크립트 | 내용 |
|---|---|
| `npm test` | E2E 122개 — 실제 마우스/키보드 조작으로 도구·단축키·대화상자·패널·커서·API·RPC 검증 |
