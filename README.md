...
- **모퉁이 위젯 (라이브 코너)** — 사각형의 모퉁이를 마우스로 직접 다룹니다.
  일러스트레이터처럼 **선택 도구와 직접 선택 도구 양쪽에서** 잡히고, 두 도구가 다르게 움직입니다.

  | 하는 일 | 결과 |
  |---|---|
  | 선택 도구(`V`)로 위젯 끌기 | 네 모퉁이가 **함께** 둥글어집니다 |
  | 직접 선택 도구(`A`)로 앵커를 고르고 끌기 | **고른 모퉁이만** 둥글어집니다 (모퉁이마다 다른 반경) |
  | 위젯 `Alt`+클릭 | 종류가 돌아갑니다 — 둥글게 → 둥글게(내부) → 모따기 |
  | 위젯 두 번 누르기 | **모퉁이** 대화상자 (종류 · 반경 · 네 모퉁이 모두) |

  직접 선택 도구로 앵커를 일부만 고르면 **그 모퉁이의 위젯만** 나타나고, 위젯 안의 표식이
  모퉁이 종류를 알려 줍니다. 이웃한 모퉁이가 변 길이를 넘어서면 CSS `border-radius` 와
  같은 방식으로 네 값을 고르게 줄입니다.
  메뉴는 **오브젝트 > 모퉁이...**, 자동화는 `set({ radius })` ·
  `set({ corners: [좌상, 우상, 우하, 좌하] })` · `set({ cornerType: 'chamfer' })` 입니다.
