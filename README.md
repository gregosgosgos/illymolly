...
- **레이어 구성** — 선택 항목을 새 레이어로 모으기, 레이어로 배포(순차), 레이어 병합
...
| **제스처** | 두 손가락 = 이동·확대(항상) · 두 손가락 탭 = 실행 취소 · 세 손가락 탭 = 다시 실행 |
...
| 스크립트 | 내용 |
|---|---|
| `npm test` | E2E 64개 — 실제 마우스/키보드 조작으로 도구·단축키·대화상자·패널·커서·API·RPC 검증 |
| `npm run test:deep` | 심층 81개 — 기하/모델/불리언 연산의 수학적 불변식을 직접 검증 |
| `npm run test:api` | 자동화 API 28개 — 헤드리스 Node, 브라우저 불필요 |
| `npm run test:mobile` | 모바일 17개 — 실제 TouchEvent 로 제스처·마킹 메뉴·타깃 크기 실측 |

```bash
npm run test:api                  # Playwright 없이 바로 실행 가능
npm i -D playwright && npx playwright install chromium
npm run test:all                  # 190개 전부
```
...
src/
  ...
  trace.js            이미지 추적 (마칭 스퀘어 · 색 양자화 · 곡선 맞춤)
...
- 효과는 가우시안 흐림 · 그림자 만들기 · 외부 광선 세 가지입니다.
  래스터 효과라 `모양 확장`으로 벡터화되지 않습니다 (확장을 시도하면 그렇게 안내합니다).
- 브러시 프리셋, 심볼, 메시, 블렌드는 미구현입니다.
- 이미지 추적은 내부적으로 긴 변 640px 로 축소해 계산하므로 아주 미세한 디테일은 사라집니다.
- 모바일에서 캔버스 회전(두 손가락 비틀기)과 스타일러스 필압은 미지원입니다.
